...
pub fn point_to_segment(point: LngLat, segment: (LngLat, LngLat)) -> f64 {
    ...
    let t = ((point.lng_deg - seg_start.lng_deg) * dx + (point.lat_deg - seg_start.lat_deg) * dy) / (dx * dx + dy * dy);
    let t = t.max(0.0).min(1.0);
    
    let projection = LngLat::new_deg(
        seg_start.lng_deg + t * dx,
        seg_start.lat_deg + t * dy
    );
...
mod tests {
    ...
    fn test_distance_euclid() {
        assert_eq!(distance_euclid(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(0.0, 0.0)), 0.0);
        assert_eq!(distance_euclid(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(3.0, 0.0)), 3.0);
        assert_eq!(distance_euclid(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(0.0, 4.0)), 4.0);
        assert_eq!(distance_euclid(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(3.0, 4.0)), 5.0);
        assert_eq!(distance_euclid(LngLat::new_deg(-1.0, -1.0), LngLat::new_deg(2.0, 3.0)), 5.0);
    ...
    fn test_distance_squared() {
        assert_eq!(distance_squared(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(0.0, 0.0)), 0.0);
        assert_eq!(distance_squared(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(3.0, 0.0)), 9.0);
        assert_eq!(distance_squared(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(0.0, 4.0)), 16.0);
        assert_eq!(distance_squared(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(3.0, 4.0)), 25.0);
        assert_eq!(distance_squared(LngLat::new_deg(-1.0, -1.0), LngLat::new_deg(2.0, 3.0)), 25.0);
    ...
    fn test_point_to_segment() {
        ...
        assert_eq!(point_to_segment(LngLat::new_deg(4.0, 5.0), zero_segment), 5.0);
    }
    ...
    fn test_point_to_segment_squared() {
        ...
        assert_eq!(point_to_segment_squared(LngLat::new_deg(2.0, 0.0), segment), 0.0);
        assert_eq!(point_to_segment_squared(LngLat::new_deg(2.0, 3.0), segment), 9.0);
        assert_eq!(point_to_segment_squared(LngLat::new_deg(-1.0, 0.0), segment), 1.0);
        assert_eq!(point_to_segment_squared(LngLat::new_deg(5.0, 0.0), segment), 1.0);
        
        let zero_segment = (LngLat::new_deg(1.0, 1.0), LngLat::new_deg(1.0, 1.0));
        assert_eq!(point_to_segment_squared(LngLat::new_deg(4.0, 5.0), zero_segment), 25.0);
    ...
    fn test_euclidean_pythagorean_theorem_verification() {
        ...
        assert!((hypotenuse - 5.0).abs() < 1e-12, "Hypotenuse error: {}", hypotenuse);
        ...
        assert!((pythagorean_check - hypotenuse).abs() < 1e-12, 
               "Pythagorean theorem violated: {}² + {}² ≠ {}²", side_a, side_b, hypotenuse);
        ...
        assert!((side_a_sq + side_b_sq - hypotenuse_sq).abs() < 1e-12,
               "Squared Pythagorean theorem violated: {} + {} ≠ {}", side_a_sq, side_b_sq, hypotenuse_sq);
    ...
    fn test_euclidean_scale_independence() {
        ...
        for scale in scale_factors {
            let scaled_triangle: Vec<LngLat> = base_triangle.iter()
            ...
            assert!((scaled_d01 - orig_d01 * scale).abs() < 1e-12,
                   "Scale {} failed for d01: {} ≠ {} * {}", scale, scaled_d01, orig_d01, scale);
            assert!((scaled_d02 - orig_d02 * scale).abs() < 1e-12,
                   "Scale {} failed for d02: {} ≠ {} * {}", scale, scaled_d02, orig_d02, scale);
            assert!((scaled_d12 - orig_d12 * scale).abs() < 1e-12,
                   "Scale {} failed for d12: {} ≠ {} * {}", scale, scaled_d12, orig_d12, scale);
            ...
            assert!((scaled_d01_sq - orig_d01_sq * scale * scale).abs() < 1e-12,
                   "Squared distance scale {} failed: {} ≠ {} * {}²", 
                   scale, scaled_d01_sq, orig_d01_sq, scale);
        }
        ...
        for scale in vec![2.0, 0.5] {
            ...
            let scaled_point = LngLat::new_deg(test_point.lng_deg * scale, test_point.lat_deg * scale);
            let scaled_pt_to_seg = point_to_segment(scaled_point, scaled_segment);
            
            assert!((scaled_pt_to_seg - orig_pt_to_seg * scale).abs() < 1e-12,
                   "Point-to-segment scale {} failed: {} ≠ {} * {}", 
                   scale, scaled_pt_to_seg, orig_pt_to_seg, scale);
        }
    }
    ...
    fn test_euclidean_coordinate_system_transformations() {
        ...
        let translated_points: Vec<LngLat> = original_points.iter()
            .map(|p| LngLat::new_deg(p.lng_deg + translation_offset.0, p.lat_deg + translation_offset.1))
            .collect();
        
        let translated_distances = vec![
        ...
        for (i, (orig, trans)) in orig_distances.iter().zip(translated_distances.iter()).enumerate() {
            assert!((orig - trans).abs() < 1e-12,
                   "Translation invariance failed for distance {}: {} ≠ {}", i, orig, trans);
        }
        
        // Test reflection invariance (flip across x-axis)
        let reflected_points: Vec<LngLat> = original_points.iter()
            .map(|p| LngLat::new_deg(p.lng_deg, -p.lat_deg))
            .collect();
        
        let reflected_distances = vec![
        ...
        for (i, (orig, refl)) in orig_distances.iter().zip(reflected_distances.iter()).enumerate() {
            assert!((orig - refl).abs() < 1e-12,
                   "Reflection invariance failed for distance {}: {} ≠ {}", i, orig, refl);
        }
        ...
        assert!((orig_pt_seg_dist - trans_pt_seg_dist).abs() < 1e-12,
               "Point-to-segment translation invariance failed: {} ≠ {}", 
               orig_pt_seg_dist, trans_pt_seg_dist);
    }
    ...
    fn test_euclidean_latitude_compression_effects() {
        ...
        for lat in test_latitudes {
            ...
            assert!((euclidean_deg - longitude_span).abs() < 1e-12,
                   "Euclidean longitude distance wrong at {}°: {} ≠ {}", lat, euclidean_deg, longitude_span);
            ...
            assert!((meridional_deg - 1.0).abs() < 1e-12,
                   "Euclidean latitude distance wrong: {} ≠ 1.0", meridional_deg);
            ...
            assert!((diagonal_deg - expected_diagonal).abs() < 1e-12,
                   "Euclidean diagonal distance wrong at {}°: {} ≠ {}", lat, diagonal_deg, expected_diagonal);
        }
        ...
        assert!((equatorial_ew - arctic_ew).abs() < 1e-12,
               "Euclidean should treat all latitudes equally: equator={}, arctic={}", 
               equatorial_ew, arctic_ew);
    }
    ...
    fn test_euclidean_projection_accuracy_limits() {
        ...
        use crate::geodesic::{haversine, VincentyError, vincenty_distance_m};
        ...
        for dist_deg in test_distances_deg {
            ...
            let euclidean_error_pct = (euclidean_approx_m - haversine_m).abs() / haversine_m * 100.0;
            
            if dist_deg <= 1.0 {
                // For small distances, error should be minimal
                assert!(euclidean_error_pct < 0.1, 
                       "Small distance error too large at {}°: {:.2}%", dist_deg, euclidean_error_pct);
            } else if dist_deg <= 5.0 {
                // For medium distances, error should be moderate
                assert!(euclidean_error_pct < 5.0,
                       "Medium distance error too large at {}°: {:.2}%", dist_deg, euclidean_error_pct);
            }
        ...
        for lat in test_latitudes {
            ...
            if lat > 0.0 {
                let equatorial_haversine = haversine(LngLat::new_deg(0.0, 0.0), LngLat::new_deg(test_distance, 0.0));
                assert!(haversine_m < equatorial_haversine,
                       "Geodesic distance should decrease with latitude: {}m < {}m at {}°",
                       haversine_m, equatorial_haversine, lat);
            }
        }
        ...
        assert!((dist_2d - 5.0).abs() < 1e-12, "2D distance in 3D space: {}", dist_2d);
        assert!((dist_3d - 13.0).abs() < 1e-12, "3D distance: {}", dist_3d);
        
        // 3D distance should be larger than 2D when z-component is non-zero
        assert!(dist_3d > dist_2d, "3D distance should exceed 2D: {} > {}", dist_3d, dist_2d);
    }
}